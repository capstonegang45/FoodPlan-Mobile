...
import 'package:image_picker/image_picker.dart';
import '../widgets/custom_bottom_nav.dart';
import '../helpers/deteksi_helper.dart'; // Import helper
...
class _DeteksiPageState extends State<DeteksiPage> {
  ...
  Future<void> _detectImage() async {
    ...
    if (result['status']) {
      setState(() {
        _detectionResult = result['prediction']; // Tampilkan hasil deteksi
        _isDetected = true;
      });
    } else {
      showDialog(
            ...
            Center(
              child: ElevatedButton(
                ...
                child: const Text(
                  'Deteksi',
                  style: TextStyle(
                      fontSize: 16, color: Color.fromARGB(255, 68, 91, 75)),
                ),
              ),
            ),
            if (_isDetected) ...[
              ...
              const SizedBox(height: 10),
              Text("Hasil Deteksi: $_detectionResult"),
            ],
          ],
        ),
      ),
